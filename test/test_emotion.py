...
class TestEmotionApi(unittest.TestCase):
  """Test out each method from the emotion recognition api."""
  @classmethod
  def setUpClass(cls):
  ...
  def test_detect_batch_emotion(self):
    """Test case for detect_batch_emotion

    Detect the emotion of a list of strings  # noqa: E501
    """
    textlist = ['HELP me pls!', "The world is on fire.", "Everything sucks, and I hate it!"] # list[str] | List of Text to classify
    lang = 'Eng' # str | Language of the input text.
    batch_text = sentim.BatchText(textlist, lang)
    api_response = self.api_instance.detect_batch_emotion(batch_text=batch_text)
    
    pprint(api_response)
    assert(isinstance(api_response, sentim.BatchEmotionResponse))
    assert(len(api_response.error_list) == 0)
    assert(len(api_response.result_list) == 3)

  def test_detect_emotion(self):
    """Test case for detect_emotion
    ...
    api_response = self.api_instance.detect_emotion(conv)

    pprint(api_response)
    assert(isinstance(api_response, sentim.EmotionResponse))
    
  def test_detect_emotion_conversation(self):
    """Test case for detect_emotion_conversation
    ...
    api_response = self.api_instance.detect_emotion_conversation(conv)
    pprint(api_response)
    assert(isinstance(api_response, sentim.BatchEmotionResponse))
    assert(len(api_response.error_list) == 0)
    assert(len(api_response.result_list) == 8)
    
  def test_ignore_first(self):
    """Test the results for calling detect_emotion_conversation with ignore_first = True
    """
    ...
    assert(api_response.result_list[0].index == 4)
